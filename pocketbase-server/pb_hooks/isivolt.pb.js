...
function authContext(e) {
  ...
  return {
    id: e.auth.id,
    name: asText(e.auth.get("name")) || e.auth.email(),
...
function validateAndApplyMovement(tool, payload, context) {
  ...
  if (type === "delivery") {
    ...
    tool.holderTechnicianId = undefined;
    tool.loanedAt = undefined;
  } else {
    ...
    Object.keys(state).forEach((key) => { tool[key] = state[key]; });
  }
...
routerAdd("POST", "/api/isivolt/entity", (e) => {
  ...
  var entity = asText(data.entity);
  var entityId = asText(data.entityId);
  if (["tools", "technicians", "accessories", "maintenance_records"].indexOf(entity) === -1 || !entityId) {
    throw new BadRequestError("Entidad o identificador no válido.");
  }

  if (asText(data.action) === "delete") {
    var existing = findEntity(e.app, context.workspace, entity, entityId);
    if (existing) e.app.delete(existing);
    appendEvent(e.app, context.workspace, entity, entityId, "delete", { id: entityId }, context.id, nowIso());
...
routerAdd("POST", "/api/isivolt/movement", (e) => {
  ...
  e.app.runInTransaction((txApp) => {
    ...
    movement.set("external_id", movementId);
    movement.set("operation_id", operationId);
    movement.set("tool_id", toolId);
    movement.set("technician_id", asText(payload.technicianId));
    movement.set("movement_type", movementType);
    movement.set("previous_status", asText(payload.previousStatus));
    movement.set("next_status", asText(payload.nextStatus));
    movement.set("payload", payload);
    movement.set("actor_user_id", context.id);
    movement.set("device_id", asText(payload.deviceId));
    movement.set("station_id", asText(payload.stationId));
    movement.set("station_nonce", asText(payload.stationNonce));
    movement.set("station_verified_at", asText(payload.stationVerifiedAt));
    movement.set("occurred_at", asText(payload.occurredAt) || nowIso());
    txApp.save(movement);
