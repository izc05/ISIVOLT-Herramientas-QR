...
export default function CloudStatus() {
  ...
  return (
    <>
      {createPortal(
        <button
          className={`cloud-status-trigger state-${state}`}
          type="button"
          onClick={() => setOpen(true)}
          aria-label={`${copy.label}. Abrir configuración de nube`}
        >
          {statusIcon(state)}
      ...
      {open && createPortal(
        <div className="cloud-modal-backdrop" onMouseDown={() => setOpen(false)}>
          <section className="cloud-modal" role="dialog" aria-modal="true" aria-label="Conexión central IsiVoltPro" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className={`cloud-summary state-${state}`}>
            ...
            {profile ? (
              <div className="cloud-session">
                <div className="cloud-user-card">
                  ...
                  <div>
                    <small>{profile.role === 'admin' ? 'ADMINISTRADOR' : profile.role === 'coordinator' ? 'COORDINADOR' : 'TÉCNICO'}</small>
                    <strong>{profile.displayName}</strong>
                    <p>{profile.email} · Espacio {profile.workspace}</p>
                  </div>
                </div>
                <div className="cloud-session-actions">
                  <button className="cloud-primary" type="button" disabled={state === 'connecting'} onClick={() => void syncNow()}>
                    <RefreshCw size={17} /> Sincronizar ahora
                  </button>
                  <button type="button" onClick={logout}><LogOut size={17} /> Cerrar sesión</button>
                </div>
              </div>
            ) : (
              <form className="cloud-login" onSubmit={login}>
                ...
                <button className="cloud-primary" type="submit" disabled={state === 'connecting'}>
                  {state === 'connecting' ? <LoaderCircle className="cloud-spin" size={18} /> : <LogIn size={18} />}
            ...
            <footer>
              <p>La copia local se conserva para poder seguir trabajando si se pierde la conexión. Al recuperarla, IsiVoltPro combina los registros y sincroniza los cambios.</p>
            </footer>
          </section>
        </div>,
        document.body,
